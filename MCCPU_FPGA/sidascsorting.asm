...
# In spring 2024
# MIPS assembly code for sorting the sid digital numbers in the acending order for FPGA.
#
##########################################################################################
...
loop1:
      ...
      add     $12, $2, $0         # bestj = i, remmember the position of the largest BCD in this loop
      add     $13, $7, $0         # tmpMax = a, remember the largest BCD in this loop
...
loop2:
      ...
      slt     $14, $13, $8        #
      beq     $14, $0, incrLoop2  # if (tmpMax >= b), increase j
      add     $13, $8, $0         #  tmpMax = b, remember the largest BCD in this loop
      add     $12, $3, $0         #  bestj = j, remmember the position of the largest BCD in this loop
incrLoop2:
      sll     $5, $5, 4           #  mask1 = mask1 << 4
      addi    $3, $3, 1           #  j = j + 1
      j       loop2
checkswap:
      slt     $14, $2, $12        #  to check if the position of the largest BCD in the this loop has been changed
      beq     $14, $0, incrLoop1
      jal     swap
incrLoop1:
...
result:
      ...
      addi    $15, $16, 28        #  put the address of label display in $15
      ori     $1, $2, 0x0004      #  $1 = 0xffff0004 # switch as input
      ori     $2, $2, 0x000c      #  $2 = 0xffff000c # seg7 as output
display:
      lw      $3, 0x00($1)
      andi    $3, $3, 0x100       #  test if bit 8 is 1
      beq     $3, $0, dispsid     #  if bit 8 = 0, display the original sid. if bit 8 = 1, display the sorted sid.
dispsortedsid:
      lw      $3, 0x184($0)       #  load the sorted student id
      j       displayseg7label
dispsid:
      lw      $3, 0x180($0)       #  load the orginal student id
displayseg7label:
